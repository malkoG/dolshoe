 ...
 */
export interface OverviewChrome {
  orgName: string;
  orgInitial: string;
  projectName: string;
 ...
 * glyphs (16px slot) so the photograph matches frame 91:2 / 91:371 without
 * editing the shared project layout.
 */
export function OverviewSidebar({ chrome }: Readonly<{ chrome: OverviewChrome }>) {
  return (
      ...
      <div className="flex flex-col gap-2 overflow-hidden px-2">
        <div className="flex flex-col gap-1">
          ...
          <nav aria-label="This project">
            <SidebarNavItem active icon={LayoutDashboard} label="Overview" />
 ...
 */
export function OverviewTopBar({ chrome }: Readonly<{ chrome: OverviewChrome }>) {
  return (
    <TopBar
      actions={
        <>
      ...
      trail={
        <>
          ...
          <BreadcrumbSeparator />
          <Breadcrumb current>Overview</Breadcrumb>
        </>
      }
    />
  );
